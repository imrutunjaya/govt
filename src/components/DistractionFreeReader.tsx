...
const DistractionFreeReader: React.FC<DistractionFreeReaderProps> = ({
  ...
  const handleDeleteUnit = (index: number) => {
    if (confirm('Are you sure you want to delete this unit?')) {
      const updatedUnits = subject.units.filter((_, i) => i !== index);
      onUpdateSubject({ units: updatedUnits });
    }
  };
  ...
  const handleSaveUnit = (unit: any) => {
    if (editingUnit) {
      ...
      updatedUnits[editingUnit.index] = unit;
      onUpdateSubject({ units: updatedUnits });
    } else {
      // Add new unit
      onUpdateSubject({ units: [...subject.units, unit] });
    }
  ...
  return (
    <>
      <div className={`fixed inset-0 ${isDarkMode ? 'bg-black' : 'bg-white'} z-50 overflow-y-auto`}>
